...
namespace plog
{
    ...
    public:
        ...
        static util::nstring format(const Record& record)
        {
            ...
            ss << record.getFile().c_str() << "@" << record.getLine() << ";";
#else
            ss << t.tm_year + 1900 << "/" << std::setfill(PLOG_NSTR('0')) << std::setw(2) << t.tm_mon + 1 << "/" << std::setfill(PLOG_NSTR('0')) << std::setw(2) << t.tm_mday << ";"
               << std::setfill(PLOG_NSTR('0')) << std::setw(2) << t.tm_hour << ":" << std::setfill(PLOG_NSTR('0')) << std::setw(2) << t.tm_min << ":" << std::setfill(PLOG_NSTR('0')) << std::setw(2) << t.tm_sec << ";"
              << getSeverityName(record.getSeverity()) << ";"
              << record.getTid() << ";"
              << record.getObject() << ";"
              << record.getFile().c_str() << ":" << record.getLine() << ";";
#endif
