...
namespace plog
{
    ...
    {
    public:
        Record(Severity severity, const char* func, const char* file, size_t line, const void* object)
            : m_severity(severity), m_tid(util::gettid()), m_object(object), m_line(line), m_file(file), m_func(func)
        {
            util::ftime(&m_time);
        }
