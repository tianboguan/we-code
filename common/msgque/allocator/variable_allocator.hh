    ...
    namespace VariableAllocatorAux {
      struct Node {
        ...
        Node join(const Node& tail_node) const {
          return (Node){tail_node.version + 1u,
                        tail_node.next,
                        uint32_t(count) + uint32_t(tail_node.count),
                        (status & uint32_t(~Node::JOIN_HEAD)) | (tail_node.status & uint32_t(~Node::JOIN_TAIL))};
        }

        Node changeNext(uint32_t new_next) const { return (Node){version+1u, new_next, count, status}; }
        Node changeCount(uint32_t new_count) const { return (Node){version+1u, next, new_count, status}; }
        Node changeStatus(uint32_t new_status) const { return (Node){version+1u, next, count, new_status}; }
      };
